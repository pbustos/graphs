...
void Graph::innerModelTreeWalk(std::uint32_t id)
{
	...
	for(auto &child_id : this->edgesByLabel(id, "RT")) 
	{
...
RMat::QVec Graph::transform(const std::uint32_t &destId, const QVec &initVec, const std::uint32_t &origId)
{
	if (initVec.size()==3)
	{
...
RMat::RTMat Graph::getTransformationMatrix(const std::uint32_t &to, const std::uint32_t &from)
{
	RMat::RTMat ret;
	...
	// {
		...
		for (auto to = listA.begin(); to != std::prev(listA.end()); ++to)
		{
			...
			ret = this->edgeAttrib<RMat::RTMat>(*(std::next(to,1)), *to, "RT") * ret;
		}
		for (auto from = listB.begin(); from != std::prev(listB.end()); ++from)
		{
			//ret = i->invert() * ret;
			ret = this->edgeAttrib<RMat::RTMat>(*from, *(std::next(from)), "RT").invert() * ret;
...
Graph::ABLists Graph::setLists(const std::uint32_t &origId, const std::uint32_t &destId)
{
 ...
 */
	std::uint32_t a_level = this->getNodeLevel(origId);
	std::uint32_t b_level = this->getNodeLevel(destId);
...
void Graph::updateTransformValues(const std::uint32_t &transformId, float tx, float ty, float tz, float rx, float ry, float rz, const std::uint32_t &parentId)
{	
//	cleanupTables();
//	InnerModelTransform *aux = dynamic_cast<InnerModelTransform *>(hash[transformId]);
	if(nodeExists(transformId) and nodeHasAttrib<std::string>(transformId, "imType", "transform"))
	{
		if(parentId != 0)
		{
			//InnerModelTransform *auxParent = dynamic_cast<InnerModelTransform *>(hash[parentId]);
			if (nodeExists(parentId) and nodeHasAttrib<std::string>(parentId, "imType", "transform"))
			{
				RTMat Tbi;
				Tbi.setTr(tx,ty,tz);
				Tbi.setR (rx,ry,rz);
				//RTMat Tpb = getTransformationMatrix( getNode (transformId)->parent->id,parentId );
                RTMat Tpb = this->getTransformationMatrix( this->getParent(transformId), parentId );
				RTMat Tpi = Tpb*Tbi;
        ...
        this->addNodeAttribs(transformId, attr);
	}
	else
	{
